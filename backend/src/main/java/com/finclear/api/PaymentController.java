...
import com.finclear.service.PaymentService; import jakarta.validation.Valid; import jakarta.validation.constraints.*; import org.springframework.http.*; import org.springframework.security.core.Authentication; import org.springframework.web.bind.annotation.*; import java.math.*; import java.util.*;
@RestController @RequestMapping("/api/v1/payments") public class PaymentController{
 ...
 @PostMapping public ResponseEntity<?> create(@RequestHeader("Idempotency-Key") String key,@Valid @RequestBody Create r,Authentication auth){var p=service.create(new PaymentService.PaymentRequest(r.accountId(),r.merchant(),r.amount(),r.currency()),key,auth.getName());return ResponseEntity.status(HttpStatus.CREATED).body(Map.of("id",p.getId(),"merchant",p.getMerchant(),"amount",p.getAmount(),"currency",p.getCurrency(),"status",p.getStatus(),"createdAt",p.getCreatedAt()));}
