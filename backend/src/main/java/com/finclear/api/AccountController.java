...
@RestController @RequestMapping("/api/v1/accounts") public class AccountController{
 ...
 @GetMapping public List<Map<String,Object>> all(Authentication a){return repo.findAllByUserEmailOrderByAccountNumber(a.getName()).stream().map(x->Map.<String,Object>of("id",x.getId(),"accountNumber",x.getAccountNumber(),"balance",x.getAvailableBalance(),"currency",x.getCurrency(),"status",x.getStatus())).toList();}
}
